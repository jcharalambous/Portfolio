 ...
 */
export function useTypewriter(script: readonly TerminalLine[], { start, instant }: Options): TypewriterState {
  ...
  useEffect(() => {
    ...
    const timers = revealPlan(script).map((step) =>
      window.setTimeout(() => setTyped({ line: step.line, chars: step.chars, done: false }), step.at),
    );
    timers.push(window.setTimeout(() => setTyped({ line: script.length, chars: 0, done: true }), revealEnd(script)));
