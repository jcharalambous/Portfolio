...
const row = "grid grid-cols-[110px_1fr] items-baseline gap-4 border-t border-white/[0.08] py-4 last:border-b";
      ...
      {facts.map((fact) => (
        <div key={fact.label} className={row}>
          ...
          <dd className="text-[17px] leading-[1.35] tracking-[-0.01em] text-ink">
            ...
            {fact.value}
            {fact.detail && <small className="mt-0.5 block text-sm text-ink-muted">{fact.detail}</small>}
          </dd>
        </div>
      ))}
      {trailing && (
        <div className={row}>
          ...
          <dd className="text-[17px] leading-[1.35] tracking-[-0.01em] text-ink">{trailing.value}</dd>
        </div>
      )}
    </dl>
  );
