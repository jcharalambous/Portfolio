...
const dotByState: Record<RailItemState, string> = {
  upcoming: "border-ink/35 bg-page group-hover:scale-125 group-active:scale-110",
  done: "border-ink/55 bg-ink/55 group-hover:scale-125 group-active:scale-110",
  active: "scale-[1.35] border-ink bg-ink group-hover:scale-[1.45] group-active:scale-[1.3]",
};
...
const dot =
  "size-[11px] rounded-full border-[1.5px] transition-[scale,background-color,border-color] duration-200 ease-strong group-hover:border-ink motion-reduce:transition-[background-color,border-color]";
const label =
    ...
    <li className="relative">
      ...
      >
        ...
        {active && (
          <span
            aria-hidden="true"
            className={`absolute top-1/2 left-1/2 size-[11px] -translate-x-1/2 -translate-y-1/2 animate-pulse-ring rounded-full border border-ink opacity-0 motion-reduce:animate-none ${onLine}`}
          />
        )}
